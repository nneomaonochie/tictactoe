...
let winning_moves
  ...
  : Position.t list
  =
  (* returns true if the position given helps the current player win the game*)
  let check_winning_combos pos = 
    let pieces = Map.set pieces ~key:pos ~data:me in
    let check_eval = evaluate ~game_kind ~pieces in
